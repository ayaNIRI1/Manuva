        ...
        <div className="my-18 text-sm text-slate-600">
                    ...
                    {product.rating?.length > 0 ? product.rating.map((item,index) => (
                        ...
                        <div className="text-center py-20 bg-slate-50/50 rounded-[40px] border border-slate-100 border-dashed max-w-2xl">
                            <p className="text-slate-400 italic">
                                {product.review_count > 0 ? t('loading_reviews') : t('no_reviews')}
                            </p>
                        </div>
                    )}
                </div>
            ...
            {product.store ? (
                <div className="flex gap-3 mt-14">
                    ...
                    <div>
                        <p className="font-medium text-slate-600">{t('product_by', { name: product.store?.name })}</p>
                        <Link href={`/shop/${product.store?.username}`} className="flex items-center gap-1.5 text-green-500"> {t('view_store')} <ArrowRight size={14} /></Link>
                    </div>
                </div>
            ) : (
                <div className="flex gap-3 mt-14">
                    ...
                    <div>
                        <p className="font-medium text-slate-600">{t('product_by', { name: product.seller_name })}</p>
