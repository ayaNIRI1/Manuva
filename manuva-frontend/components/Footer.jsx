    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                ...
                // Take only first 6 for footer
                setDynamicCategories(data.slice(0, 6));
            } catch (error) {
    ...
    const linkSections = [
        {
            ...
            links: dynamicCategories.length > 0 
                ? dynamicCategories.map(cat => ({
                    text: getShortName(cat.name),
                    path: `/shop?category=${cat.id}`,
                    icon: null
                  }))
                : [
                    { text: language === 'ar' ? "الفخار" : "Pottery", path: '/shop?category=pottery', icon: null },
                    { text: language === 'ar' ? "المنسوجات" : "Textiles", path: '/shop?category=textiles', icon: null },
                    { text: language === 'ar' ? "النحاس" : "Copper", path: '/shop?category=copper', icon: null },
                    { text: language === 'ar' ? "الجلود" : "Leather", path: '/shop?category=leather', icon: null },
                ]
        },
        {
            ...
            links: [
                ...
                { text: t('privacy_policy'), path: '/privacy', icon: null },
                { text: t('become_artisan'), path: '/create-store', icon: null },
                { text: t('about'), path: '/about', icon: null },
            ]
        },
        {
            title: t('contact_us'),
            links: [
                { text: "smanuva85@gmail.com", path: 'mailto:smanuva85@gmail.com', icon: MailIcon },
                { text: "0666378037", path: 'tel:0666378037', icon: PhoneIcon },
                { text: t('footer_location'), path: '/', icon: MapPinIcon }
            ]
        }
    ];
